...
export const CurrentWeather = () => {
    ...
    return (
      <Card className="w-full max-w-md mx-auto text-center p-8">
        <div className="text-muted-foreground">
          ...
          <p className="text-lg">
            Search for a city or use your location to get weather information
          </p>
        </div>
      </Card>
    );
  ...
  // ✅ Use WeatherAPI’s condition code mapped to icon/label
  ...
  const windSpeedDisplay = units === "metric"
    ...
    : `${Math.round(weatherData.current.windSpeed * 2.237)} mph`; // m/s to mph
      ...
      <div className={`${weatherInfo.bgClass} p-6 text-white transition-all duration-500`}>
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-white/80">{weatherData.location.country || "Your Location"}</p>
          </div>
      ...
      <CardContent className="p-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="bg-accent/50 rounded-lg p-3 text-center">
            ...
            <span className="font-medium">{windSpeedDisplay}</span>
          </div>
